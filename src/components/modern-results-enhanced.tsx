...
export function ModernResultsEnhanced({ result, onReset, shareUrl }: ModernResultsEnhancedProps) {
  ...
  const handleShare = async () => {
    ...
    const shareTitle = `AI Visibility Analysis: ${siteResult.title}`
    const shareText = `🤖 Just analyzed ${siteResult.title} for AI visibility! Score: ${siteResult.overallScore}/100. See how ChatGPT, Claude & Perplexity can find this site:`
  ...
  const handleEmail = () => {
    ...
    const highImpactRecs = siteResult.recommendations.filter((r: any) => r.impact === 'high').length;
    const quickWins = siteResult.recommendations.filter((r: any) => r.effort === 'easy').length;
    
    const subject = encodeURIComponent(`🤖 AI Visibility Analysis: ${siteResult.title} (Score: ${siteResult.overallScore}/100)`)
...
• ${siteResult.title}
• URL: ${siteResult.url}
• Overall AI Visibility Score: ${siteResult.overallScore}/100
• Analysis completed: ${new Date().toLocaleDateString()}

📈 CATEGORY BREAKDOWN:
• AI Access Control: ${siteResult.categories.aiAccess}/100
• Content Structure: ${siteResult.categories.contentStructure}/100
• Technical Infrastructure: ${siteResult.categories.technicalInfra}/100
• Structured Data: ${siteResult.categories.structuredData}/100

🎯 IMPROVEMENT OPPORTUNITIES:
• ${highImpactRecs} high-impact recommendations
• ${quickWins} quick wins available
• ${siteResult.recommendations.length} total improvements identified

🤖 WHAT THIS MEANS:
This analysis shows how well your website can be discovered and cited by AI platforms like ChatGPT, Claude, and Perplexity. Higher scores mean better AI visibility.
...
${siteResult.recommendations.length > 0 ? `🔧 TOP RECOMMENDATIONS:
${siteResult.recommendations.slice(0, 3).map((rec: any, i: number) => `${i + 1}. ${rec.title} (${rec.impact} impact, ${rec.effort} effort)`).join('\n')}` : '✅ Great job! No major issues found.'}

View full detailed report: ${window.location.href}

---
Generated by Am I Visible on AI - Free AI Visibility Checker
